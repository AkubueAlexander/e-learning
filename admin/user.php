<?php 
    ...
    if (isset($_POST['edit'])) {
        ...
        $sql = "UPDATE user SET fullName = :fullName, email = :email, pass = :pass, verifiedStatus = :verifiedStatus WHERE id = :id";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([
            ':fullName' => $fullName,
            ':email' => $email,
        ...
        header('location: user?updated=true');
        exit;
    }
...
<body class="bg-gray-50">
            ...
            <div class="flex-1 overflow-auto p-6 readable-font">
                <div class="max-w-7xl mx-auto bg-white p-6 rounded-xl shadow" x-data="userTable()" x-init="init()">
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
                        <h1 class="text-xl font-semibold text-gray-800">User History</h1>
                        ...
                        <table class="min-w-max divide-y divide-gray-200 text-sm">
                            <thead class="bg-gray-100 text-gray-700 font-semibold">
                                <tr>
                                    ...
                                    <th class="px-4 py-2 text-left">Verified Status</th>
                            ...
                            <tbody>
                                <template x-for="row in paginatedRows()" :key="row.id">
                                    <tr class="hover:bg-gray-50 border-b">
                                        ...
                                        <td class="px-4 py-2" x-text="row.pass"></td>
                                ...
                                <form action="" method="post">
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            ...
                                            <input type="pass" x-model="modalData.pass" name="pass"
                                                class="w-full border rounded-lg p-2" :readonly="!isEditing"
                                                :class="!isEditing ? 'bg-gray-50' : ''">
                                        </div>
                                        <div>
                                            <label class="block text-sm font-medium mb-1">Verified Status</label>
                                            <input type="number" step="1" x-model.number="modalData.verifiedStatus"
                                                name="verifiedStatus" class="w-full border rounded-lg p-2"
    ...
    const userFromPHP = <?php echo json_encode($rows, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES); ?>;
    ...
    function userTable() {
        return {
            ...
            init() {
                this.rows = userFromPHP.map(user => {
                    return {
                        id: user.id,
