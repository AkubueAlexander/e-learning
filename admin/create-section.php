<?php 
    ...
    if (isset($_POST['submit'])) {
        ...
        $sql = 'INSERT INTO section (courseId, title, position) VALUES (:courseId, :title, :position)';
        $stmt = $pdo->prepare($sql);
        $stmt->execute([
            ':courseId' => $courseId,
            ':title' => $title,
        ...
        // Redirect to section page after successful insertion
        header('Location: section?created=true');
        exit();
    }
                ...
                <form action="" method="post" enctype="multipart/form-data">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="mb-6 md:col-span-2">
                                <label for="courseId" class="block text-gray-800 text-sm font-semibold mb-2">
                                    Select Course
                                </label>

                                <div class="relative">
                                    <select name="courseId" id="courseId" required class="w-full appearance-none px-4 py-3 bg-white border border-gray-300 rounded-xl shadow-sm 
                                    ...
                                    transition duration-200 ease-in-out cursor-pointer">
                                        ...
                                        <option value="<?php echo $row->id; ?>"
                                            <?php echo isset($_POST['courseId']) && $_POST['courseId'] == $row->id ? 'selected' : ''; ?>>
                                            <?php echo $row->title; ?>
                                        </option>
                                        <?php endforeach; ?>
                                    </select>
