...
if (isset($_POST['edit'])) {
       ...
       $sqlUpdate = "UPDATE lesson SET sectionId = :sectionId, title = :title, position = :position, contentUrl = :contentUrl,
        contentType = :contentType, duration = :duration WHERE id = :id";
        $stmtUpdate = $pdo->prepare($sqlUpdate);
        $stmtUpdate->bindParam(':id', $id);
        $stmtUpdate->bindParam(':sectionId', $sectionId);        
        $stmtUpdate->bindParam(':title', $title);
        $stmtUpdate->bindParam(':position', $position);
        $stmtUpdate->bindParam(':contentUrl', $contentUrl);
        $stmtUpdate->bindParam(':contentType', $contentType);
        $stmtUpdate->bindParam(':duration', $duration);       
        $stmtUpdate->execute();



    
       header("Location: lesson?updated=true");
        exit;
...
<body class="bg-gray-50">
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <div class="flex flex-col flex-1 overflow-hidden">
            ...
            <div class="flex-1 overflow-auto p-6 readable-font">
                <div class="max-w-7xl mx-auto bg-white p-6 rounded-xl shadow" x-data="lessonTable()" x-init="init()">
                 ...
                 <div class="flex justify-end mb-4">
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
                        <h1 class="text-xl font-semibold text-gray-800">Lesson History</h1>
                        ...
                        <table class="min-w-max divide-y divide-gray-200 text-sm">
                            <thead class="bg-gray-100 text-gray-700 font-semibold">
                                <tr>
                                    ...
                                    <th class="px-4 py-2 text-left">Content Url</th>
                            ...
                            <tbody>
                                <template x-for="row in paginatedRows()" :key="row.id">
                                    <tr class="hover:bg-gray-50 border-b">
                                        ...
                                        <td class="px-4 py-2" x-text="row.sectionId"></td>
                                        <td class="px-4 py-2" x-text="row.title"></td>                                                                              
                                        <td class="px-4 py-2" x-text="row.position"></td>
                                        <td class="px-4 py-2" x-text="row.contentUrl"></td>
                                        <td class="px-4 py-2" x-text="row.contentType"></td>                                                                              
                                        <td class="px-4 py-2" x-text="row.duration"></td>
                            ...
                            <div class="bg-white w-[95%] max-w-2xl rounded-xl shadow-lg p-6">
                                ...
                                <form action="" method="post">
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            <label class="block text-sm font-medium mb-1">Section Id</label>
                                            <input type="number" step="1" x-model.number="modalData.sectionId" name="sectionId"
                                        ...
                                        <div>
                                            ...
                                            <input type="number" step="1" x-model.number="modalData.position" name="position"
                                                class="w-full border rounded-lg p-2" :readonly="!isEditing"
                                                :class="!isEditing ? 'bg-gray-50' : ''">
                                        </div>
                                         <div>
                                            <label class="block text-sm font-medium mb-1">Content Url</label>
                                            <input type="text" x-model="modalData.contentUrl" name="contentUrl"
                                                class="w-full border rounded-lg p-2" :readonly="!isEditing"
                                                :class="!isEditing ? 'bg-gray-50' : ''">
                                        </div>
                                         <div>
                                            <label class="block text-sm font-medium mb-1">Content Type</label>
                                            <input type="text" x-model="modalData.contentType" name="contentType"
                                                class="w-full border rounded-lg p-2" :readonly="!isEditing"
                                                :class="!isEditing ? 'bg-gray-50' : ''">
                                        </div>                                                                               
                                        ...
                                        <div>
                                            ...
                                            <input type="text" x-model="modalData.duration" name="duration"
                                                class="w-full border rounded-lg p-2" :readonly="!isEditing"
                                                :class="!isEditing ? 'bg-gray-50' : ''">
                                        </div>
    ...
    const lessonFromPHP = <?php echo json_encode($rows, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES); ?>;
    ...
    function lessonTable() {
        return {
            ...
            init() {
                this.rows = lessonFromPHP.map(lesson => {
                    return {
                        id: lesson.id,
            ...
            saveEdit() {
                ...
                if (i !== -1) {
                    ...
                    this.rows[i] = {
                        ...this.rows[i],
                        sectionId: this.modalData.sectionId,
                        title: this.modalData.title,
                        position: this.modalData.position,
                        contentUrl: this.modalData.contentUrl,
                        contentType: this.modalData.contentType,
                        duration: this.modalData.duration,
